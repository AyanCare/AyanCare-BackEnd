...
const endereco_pacienteDAO = require('../model/DAO/endereco_pacienteDAO.js')
...
const getEnderecoByID = async function (id) {
    if (id == '' || isNaN(id) || id == undefined) {
        ...
        let dadosEndereco = await endereco_pacienteDAO.selectEnderecoById(id)
...
const insertEndereco = async function (dadosEndereco) {
    ...
    if (
        ...
        dadosEndereco.idPaciente == '' || dadosEndereco.idPaciente == undefined || isNaN(dadosEndereco.idPaciente)
    ) {
        return messages.ERROR_REQUIRED_FIELDS
    } else {
        let resultDadosEndereco = await endereco_pacienteDAO.insertEndereco(dadosEndereco)

        if (resultDadosEndereco) {
            let novoEndereco = await endereco_pacienteDAO.selectLastId()
            await pacienteDAO.updateEnderecoPaciente(novoEndereco.id, dadosEndereco.idPaciente)
...
const updateEndereco = async function (dadosEndereco, id) {
    if (
        ...
        let atualizacaoEndereco = await endereco_pacienteDAO.selectEnderecoById(id)

        if (atualizacaoEndereco) {
            let resultDadosEndereco = await endereco_pacienteDAO.updateEndereco(dadosEndereco)
