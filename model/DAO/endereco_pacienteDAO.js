/**************************************************************************************
 * Objetivo: Responsável pela manipulação de dados dos Enderecos dos Pacientes no Banco de Dados.
 * Data: 11/06/2023
...
const selectEnderecoById = async function (idEndereco) {
    let sql = `SELECT * FROM tbl_endereco_paciente where id = ${idEndereco}`
...
const selectLastId = async function () {
    let sql = 'select * from tbl_endereco_paciente order by id desc limit 1;'
...
const insertEndereco = async function (dadosEndereco) {
    let sql = `insert into tbl_endereco_paciente(
        logradouro,
        bairro,
        cep,
        numero,
...
const updateEndereco = async function (dadosEndereco) {
    let sql = `update tbl_endereco_paciente set
